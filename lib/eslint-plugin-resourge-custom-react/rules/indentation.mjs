...
const ruleName = 'resourge/indentation';
...
const rule = (primary, secondaryOptions = {}, context) => {
	return (root, result) => {
		...
		const isStyledComponents = root.raws && Object.keys(root.raws).some((key) => key.includes('styled'));
		...
		root.walk((node) => {
			...
			const nodeLevel = indentationLevel(node, isStyledComponents ? 1 : 0);
...
function getRootBaseIndentLevel(root, baseIndentLevel, space) {
	if (root.inputs && root.inputs.length && root.inputs[0].file.includes('.ts')) {
		return 1;
	}
